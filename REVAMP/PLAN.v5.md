# HealthMadeClear Launch Revamp — Implementation Plan v5

**Status:** Contract for all implementation work in the 4-day launch window. **Supersedes `REVAMP/PLAN.v4.md` (v4).** v3, v2, and v1 are historical.  
**Date:** 2026-08-27  
**Inputs:** `REVAMP/AUDIT-CODE.md` (file audit + independent adversarial/live-DB audit), `REVAMP/AUDIT-VISUAL.md`, `REVAMP/CRITIQUES/ROUND-1.md`, `REVAMP/CRITIQUES/ROUND-2.md`, `REVAMP/CRITIQUES/ROUND-3.md`, `REVAMP/CRITIQUES/ROUND-4.md` (panel + Cursor/Grok second opinion)  
...
v4 already incorporated ROUND-1/2/3 + CF-1…CF-23. Do not re-litigate those without a new finding. This file starts from that contract.

---

## Changelog from v4

Every row is a delta vs `REVAMP/PLAN.v4.md`. Critiques: `Staff` / `Sec` / `UX` / `Clin` / `PM` = panel ROUND-4; `CF-N` = combined flaws in the Cursor/Grok second opinion on ROUND-4 (CF-24…CF-32).

| Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Driven by                                          |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| This file is the contract; v4 is historical.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Process                                            |
| **`handleQuizAttemptSideEffects` takes `locale`** (and the same localize callback as the lesson fn). `mutations.ts` passes them on skip-lower **and** upsert-success. `saveQuizAttempt` deps include `locale`. `checkAndAwardAchievements` returns ids only — **no** English notification inserts. Hook builds toast + notification copy via `useTranslations("achievements")` (reuse existing `unlocked` + `items.{id}`; do **not** add a duplicate `progress.achievementUnlocked`). Client `sideEffects` must **not** import `getMessages` / `getLocalizedAchievement`. Import `updateDailyLog` from `@/lib/dashboard/dailyLog`, not the `export *` barrel. | **Staff 🔴 1**, **Staff 🔴 2 PARTIAL**, **CF-24**  |
| **`clearLocalHealthData` + `AppProviders.resetLocalProgress`.** Wipe health keys in localStorage **and** sessionStorage (guest prefix + UI progress + checklist + planner v1/v2 + glossary lookups). Do **not** wipe locale/theme/textSize/simpleMode. Empty React state **before** `removeItem` (persist effect otherwise rewrites). Settings delete `finally` **and** `AuthProvider.signOut` both call `resetLocalProgress`. Phase 9 (Day 1, after P2) owns the util — P9 is **not** “if time.” P5 signOut uses the same fn.                                                                                                                                | **Sec 🔴 1**, **CF-20 leftover**, **CF-25**        |
| Compact-at-xl **keeps** icon-only login. **Require** `aria-label={authT("loginButton")}` (not `nav.login` — key does not exist). Unit/Playwright: named link at 1280. Stop `Header.test` from mocking away `aria-label`. Do **not** restore label text at `xl`.                                                                                                                                                                                                                                                                                                                                                                                               | **UX 🔴 1 PARTIAL**, **CF-21 leftover**, **CF-26** |
| New paste-ready `homeCarePediatricNote` EN+ES under the home-care card (not a checklist row). Qualify/remove unqualified “Low fever.” Tests: infant / 3 months / dehydrat + ES. 911-class exception (same as airway / atypical ACS).                                                                                                                                                                                                                                                                                                                                                                                                                          | **Clin 🔴 1**, **CF-27**                           |
| **Gate 0:** forbid `npx supabase db push` until Netlify `SUPABASE_SERVICE_ROLE_KEY` is proven (`netlify env:get` or dashboard screenshot in the PR). Phase 3 env-gate **file** may merge first. 014 contact revoke stays last. No AM/PM split.                                                                                                                                                                                                                                                                                                                                                                                                                | **PM 🔴 1 PARTIAL**, **CF-28**                     |
| Rollback: drop unique while any upsert client is live = **42P10** (HEAD `guestProgress` + P6 `mutations`). Revert P6 client **before** `015_emergency.sql`. If P5 merged, revert P5 first. Unique-live + `.insert()` = **23505** stays.                                                                                                                                                                                                                                                                                                                                                                                                                       | **PM 🔴 2 PARTIAL**, **CF-8 leftover**             |
| `PLANNER_DEFAULTS_BY_TYPE` lives **inside** `useVisitPlanner.ts`. `changeVisitType(next)` looks up the map internally. Parent does **not** pass a `defaultQuestions` array that changes with `visitType`.                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **Staff 🟡 1**, **CF-29**                          |
| Contact body: stream-read with byte counter; 413 if > ~10KB; then parse. If `Content-Length` > 10KB, 413 without reading. Never unbounded `request.json()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **Sec 🟡 1**, **CF-30**                            |
| Sentry: strip `?` **and** `#` from `event.request.url` and breadcrumb URLs. Keep path slugs. Privacy `collectBodyErrors` names query **and** fragment. Test `#access_token=` gone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **Sec 🟡 2**, **CF-31**                            |
| Glossary mobile A-Z: right-edge fade mask (`mask-image`) so H–Z are discoverable at 390px.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **UX 🟡 3**                                        |
| Care-guide print: keep `no-print` on the red banner. Add a **print-only** 911/988 line. Printable cards still include P4 `emergencyBody` / `whenInDoubtBody`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **Clin 🟡 3 PARTIAL**                              |
| Validator denylist adds exact/whole-string `Internet`, `Google`, `N/A`, `None`, `Unknown`. Do **not** add `Search` / `Online`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **Clin 🟡 1 PARTIAL**                              |
| Day 4 after P14 timebox: **Phase 16 must-dos (19.1), then Phase 15.** Never drop 19.1. P15 may slip.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **PM 🟡 3**, **CF-32**                             |
| Header/dashboard metadata `display_name`: `trim()` then `\|\|` fallback (empty/whitespace). Not a schema change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **Staff 🟡 2 PARTIAL** (cheap; not 🔴)             |

**Rejected with justification (not in the table as a code change):**

- Phase 7 `getMessages` is a new dual-catalog leak that violates Phase 14 — 15 client files already import `@/lib/i18n`; P14 is content barrels, not message catalogs. (`Staff 🔴 2` as stated)
- Restore login text at `xl` / Tools dropdown / `t("nav.login")` — fights CF-21 overflow; key does not exist; icon is `LogIn` not User. (`UX 🔴 1` as stated)
- Day 1 morning/afternoon contact 500 — §4.2.4 already names the key; form 503s today without it. (`PM 🔴 1` as stated)
- “P5 introduces quiz upsert” — HEAD `guestProgress` already upserts; C23/CF-8. (`PM 🔴 2` as stated)
- Article TOC scroll-spy as launch 🔴 — polish, not WCAG. (`UX 🟡 1`)
- Denylist `Search` / `Online` / `Online Search` / `Various` — false-fail real citations. (`Clin 🟡 1` extras)
- `display_name` NULL crashes profiles UI — every profiles consumer already fallbacks. (`Staff 🟡 2` as stated)
- Force `@/lib/i18n` catalog split into the P14 3h timebox. (`Staff 🔴 2` implied)

**Still in force from v4 (ROUND-1/2/3 / CF-1…CF-23), not re-opened:**
...
- Contact route keeps origin / rate-limit / honeypot; **stream** body cap (v5 tightens the how).
...
- Planner per-type default map (now **inside** the hook); `xl:flex` + compact-at-xl; HomeClient (not Hero) owns the video.
...
- Account delete: local `signOut({ scope: "local" })` in `finally` (v5 **adds** health-key wipe + React reset).
...
| Account deletion   | **Deploy `delete_user`**, then **local** `signOut({ scope: "local" })` in `finally` (ignore logout errors). **Then `resetLocalProgress()`** (empty AppProviders React state **then** `clearLocalHealthData()`). Same wipe on **logout**. Contact rows stay (not user-linked). Privacy copy must say that **and** how to request contact erasure (`privacy@healthmadeclear.com`).                                                           | GDPR/CCPA control must work on cookies **and** shared-device `localStorage`, not only in Postgres. `removeItem` without resetting mounted `AppProviders` is rewritten by the persist effect. |
...
| ADV-09 `delete_user` missing                                             | 🔴      | 1 + **9 (client finally + health wipe)**                          | Throwaway RPC that deletes `auth.users` is a **gate**. Settings: local `signOut` in `finally` + `resetLocalProgress`.                                                         |
...
| ADV-12 care-guide treatment voice                                        | 🔴      | 4                                                                 | Atypical-ACS chest-pain spec strings + one 988 sentence + `homeCarePediatricNote`                                                                                             |
...
| TEST-01 auth form unit tests                                             | 🟡      | 9                                                                 | **Day 1 after Phase 2** (required — delete wipe lives here).                                                                                                                  |
| Visual header `2xl` → **`xl`** (not `lg`)                                | 🔴 UX   | 10                                                                | Compact-at-xl. 8 nav items. Login icon-only at `xl` **with** `aria-label={authT("loginButton")}`. Playwright no overflow 1280/1440 **on `/en` and `/es`**. No Tools dropdown. |
...
| Glossary A-Z horizontal snap                                             | 🟡      | 11                                                                | Plus right-edge fade mask on mobile (`scrollbar-none` has no overflow cue).                                                                                                   |
...
| Print CTAs lesson/article/care-guide                                     | 🟡 feat | 15                                                                | Print-only 911/988 line on care-guide. Red banner stays `no-print`.                                                                                                           |
...
| Day       | Phases                                                                      | Theme                                                                                                                                                                                          |
| --------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Day 1** | **1 (`014` only, Gate 0 first)**, 2, 3, **9 (after P2)**                    | Security + auth + legal copy + account-delete client wipe. **Do not announce launch until 1–3 are on production.** **Do not apply `015`.** P9 is **not** optional.                             |
...
| **Day 4** | 13 (if leftover), **14 (descoped, 3h timebox)**, **16 must-dos (19.1)**, 15 | Reading UX, **generator + client** split, **then CSP/Sentry PII**, then print/share polish. P14 is **not** the first task of the morning. Stop P14 at 13:00 if not green. **Never drop 19.1.** |
...
| `src/app/api/contact/route.ts`               | **Keep** Origin check, `checkRateLimitDistributed`, `website` honeypot, `EMAIL_REGEX`, field length limits. **Do not rewrite.** Add a **stream byte cap** (~10KB): if `Content-Length` is present and `> 10240`, return 413 **without** reading the body. Otherwise read `request.body` with a running byte count; if count exceeds 10240, 413 and abort. Then `JSON.parse` the buffered bytes. **Never** `await request.json()` on an unbounded body. Do **not** add `hp_company`. Idempotency-Key remains optional (memory LRU only). |
...
- Existing `src/app/api/contact/route.test.ts` — still pass (origin 403, honeypot fake-success, rate limit). Add: oversized body → 413 (include a case **without** `Content-Length`, e.g. a mocked stream, so the after-parse hole cannot regress).
...
| `homeCare`, `homeCareBody`, `homeCareChecklist`, **`homeCarePediatricNote` (new)** | Checklist is educational examples, not “if you have X, stay home.” Do **not** list “Low fever” unqualified. New note under the card (not a pipe row): infant fever + pediatric dehydration are 911-class. |
...
- `homeCarePediatricNote` EN: "Fever in infants under 3 months, or signs of dehydration in young children (no wet diapers for 8 hours, crying with no tears, sunken eyes, extreme sleepiness), are treated as emergencies in US emergency departments — not as home monitoring. In the United States, call 911."
- `homeCarePediatricNote` ES: same clinical content; fiebre en bebés menores de 3 meses; deshidratación / pañales secos / llanto sin lágrimas / somnolencia extrema; no observación en el hogar; `911` + US qualifier.
...
- [ ] `homeCarePediatricNote` (EN+ES) names infant fever (<3 months) and pediatric dehydration; rendered under the home-care card. Checklist has no bare “Low fever” / “Fiebre baja.”
...
| `src/components/providers/AuthProvider.tsx`                           | After `supabase.auth.signOut()` succeeds, call `useAppState().resetLocalProgress()` (P9 util — **required on Day 1**). Do **not** add a 2-hour migrate TTL (browse-then-signup is the product). Shared-kiosk confirm dialog is **out of scope**. If this PR merges before P9 (should not), create `resetLocalProgress` / `clearLocalHealthData` here rather than a second util.                                                                                                                                    |
...
- `AuthProvider.test.tsx` — `signOut` calls `resetLocalProgress` (mock). After logout, `STORAGE_KEYS.completedLessons` is empty and `hmc-theme` remains.
...
**Goal:** Signup does not enumerate emails; expired sessions get a clear message; critical auth forms have unit tests; **account delete and (via the same util) logout wipe shared-device health storage without the persist-effect rewrite.**
...
| `src/app/[locale]/glossary/GlossaryClient.tsx`                    | Mobile `< sm`: `flex flex-nowrap overflow-x-auto snap-x snap-mandatory` + `scrollbar-none`; each letter `snap-center min-h-11 min-w-11 shrink-0`. Add a right-edge fade (`mask-image: linear-gradient(to right, black 85%, transparent 100%)` or equivalent) so H–Z are discoverable when A–G fill 390px. Desktop can keep wrap. `showAlphabet` unchanged. |
...
- [ ] Glossary A-Z does not wrap into a 26-button grid on 390px. Mobile row has a right-edge overflow cue (fade mask).
...
**Dependencies:** After correctness phases. **Day 4, not first thing in the morning.** Hard **3-hour** timebox: if analyzer/CI not green by **13:00**, stop; keep whatever lesson/quiz client split is done; drop remaining path/glossary generator work; proceed to **Phase 16 must-dos (19.1), then Phase 15**. Do **not** move this rewrite to Day 2. Message catalogs (`@/lib/i18n` dual JSON) are **out of this timebox**.
...
- [ ] Lesson, article, care-guide have Print; `@media print` hides `no-print`. Printable surfaces use `print-color-adjust: exact`. Care-guide print includes a print-only 911/988 line; red banner stays `no-print`.
...
**A11y:** TOC `nav aria-label={t("onThisPage")}`; skip link already to `#main-content`. **No scroll-spy required this window** (IntersectionObserver is polish, not WCAG).
...
The revamp is done when **all** of the following are true. Phase 16 AppProviders-split and N+1 may be unchecked if explicitly dropped in a Day-4 slip note on this file. **Phase 16 §19.1 (CSP + Sentry PII) may not be dropped.** Phase 14 descoped split must still have analyzer evidence **or** an explicit slip note. Phase 15 print/share may slip.
...
- [ ] `delete_user` exists; Settings deletion removes auth user **and** clears local session even if GoTrue logout errors (spot-check on a throwaway account). **Health `localStorage` keys gone; theme/locale kept.** Persist effect does not restore the previous user. Logout uses the same wipe.
- [ ] Gate 0: Netlify `SUPABASE_SERVICE_ROLE_KEY` proven before 014 `db push`. Contact `/api/contact` stream-caps bodies (~10KB) even without `Content-Length`.
...
- [ ] Home-care pediatric note names infant fever (<3 months) and pediatric dehydration in EN+ES; checklist has no bare “Low fever.”
...
- [ ] 1440px header shows desktop nav (`xl`, not `lg`). Header does not overflow at 1280 or 1440 on `/en` **and** `/es`. Login at `xl` is icon-only **with** `aria-label={auth.loginButton}`.
...
P2 (auth routes, session-aware reset) ──► P9 (delete wipe + resetLocalProgress; Day 1)
...
P16 19.1 (CSP + Sentry PII, strip ? and #) after P14 timebox
