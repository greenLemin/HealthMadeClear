# HealthMadeClear Launch Revamp — Implementation Plan v4

**Status:** Contract for all implementation work in the 4-day launch window. **Supersedes `REVAMP/PLAN.v3.md` (v3).** v2 and v1 are historical.  
**Date:** 2026-08-27  
**Inputs:** `REVAMP/AUDIT-CODE.md` (file audit + independent adversarial/live-DB audit), `REVAMP/AUDIT-VISUAL.md`, `REVAMP/CRITIQUES/ROUND-1.md`, `REVAMP/CRITIQUES/ROUND-2.md`, `REVAMP/CRITIQUES/ROUND-3.md` (panel + Cursor/Grok second opinion)  
...
v3 already incorporated ROUND-1/2 + CF-1…CF-16. Do not re-litigate those without a new finding. This file starts from that contract.

---

## Changelog from v3

Every row is a delta vs `REVAMP/PLAN.v3.md`. Critiques: `Staff` / `Sec` / `UX` / `Clin` / `PM` = panel ROUND-3; `CF-N` = combined flaws in the Cursor/Grok second opinion on ROUND-3 (CF-17…CF-23).

| Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Driven by                               |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| This file is the contract; v3 is historical.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Process                                 |
| **Quiz score contract (Option A):** persist `score` = correct count, `max_score` = question count. `passed = max_score > 0 && score / max_score >= 0.7` (MDX `passScore: 70`). `QuizClient` keeps a **local** percent for UI (`score >= quiz.passScore`) and calls `saveQuizAttempt(quiz.id, lessonId, correctCount, total, answers)`. `recordQuizScore` still stores **percent** on guest `QuizScore` (do not merge shapes). Shared `src/lib/quizScore.ts`: `isQuizPassed`, `toPercent`, `normalizeStoredScore`. | **Staff 🔴 1**, **CF-17**               |
| **015 before dedupe:** `UPDATE quiz_attempts SET score = LEAST(max_score, GREATEST(0, ROUND(score * max_score / 100.0)::int)) WHERE max_score > 0 AND score > max_score;` then recalc `passed`. Then existing highest-score dedupe + unique. Without this, unique freezes 80/5 forever and CF-13 skip-lower never writes 4/5.                                                                                                                                                                                     | **Staff 🔴 1**, **CF-17**               |
| Phase 6 scope adds `QuizClient.tsx`, `src/lib/quizScore.ts`, `dashboard/quizzes.ts` / `activity.ts` only if they assume inverted units, and production-shaped tests. Guest migrate uses `normalizeStoredScore` (Phase 5 calls the P6 helper).                                                                                                                                                                                                                                                                     | **Staff 🔴 1**, **CF-17**               |
| Completed-lessons quiz lookup: `.in("quiz_id", [lessonId, lessonId + "-quiz"])`. Do **not** rewrite stored `quiz_id` values (`quiz.id === lessonId` in bundles). Keep `replace("-quiz")` as a no-op-safe map to lesson id. Tests include a live-shaped id **and** a legacy `-quiz` row.                                                                                                                                                                                                                           | **Staff 🔴 2**, **CF-18**               |
| `ResetPasswordClient`: `exchangedRef` consume-once; after detecting `code` / `token_hash`, `history.replaceState` to pathname (locale path kept). Tests: two effect runs → one exchange. **Not** framed as a production StrictMode bug.                                                                                                                                                                                                                                                                           | **Staff 🔴 3 PARTIAL**, **CF-19**       |
| Account delete: after `rpc("delete_user")` success, `supabase.auth.signOut({ scope: "local" })` in `try/finally`. Ignore logout errors. Always clear guest keys + redirect. **Do not** call `AuthProvider.signOut` on this path (global logout + navigate can throw before cleanup). Unit test: rpc ok + signOut rejects → still redirected, session empty.                                                                                                                                                       | **Sec 🔴 1**, **CF-20**                 |
| Header: **keep** compact-at-xl + 8 items. **Reject** Tools dropdown / 4-item `xl` list (panel used labels that are not in `getNavItems`). Playwright overflow at 1280 **and** 1440 on **`/es` as well as `/en`**. Also hide signup at `xl` (`2xl:hidden` → `xl:hidden`). Dashboard tests assert raw `averageQuizScore === 80`; `clampPercent` would hide 1600 as 100.                                                                                                                                             | **UX 🔴 1 REJECT as stated**, **CF-21** |
| `scenarioChestPainBody` EN+ES is paste-ready atypical-ACS + don’t-wait / don’t-self-treat + US 911 (same 911-class exception as sore-throat airway flags). Tests: jaw/neck/back/sweat/911, not only arm.                                                                                                                                                                                                                                                                                                          | **Clin 🔴 1**, **CF-22**                |
| Phase 13 owns HomeClient video **order +** `preload="none"` + `prefers-reduced-motion` (no autoplay, poster). Phase 16 **drops** `HomeClient.tsx`. Scope not cut.                                                                                                                                                                                                                                                                                                                                                 | **PM 🔴 1**, **CF-23**                  |
| `privacy.collectBodyContact`: add erasure instruction via existing `privacy.privacyEmail` (`privacy@healthmadeclear.com`). Account delete still does not auto-wipe contact rows.                                                                                                                                                                                                                                                                                                                                  | **Sec 🟡 2**                            |
| `reportServerError` HTTP ingest: in-memory sliding window, max 5 posts / 10s / isolate. Overflow → `console.error` only.                                                                                                                                                                                                                                                                                                                                                                                          | **Sec 🟡 3**                            |
| Printable surfaces: `print:border-neutral-900 print:text-black print:[print-color-adjust:exact]` (planner Step 3, care-guide, PrintButton CSS). No PDF library.                                                                                                                                                                                                                                                                                                                                                   | **UX 🟡 3**                             |
| Care-guide `emergencyBody` + `whenInDoubtBody`: one US-qualified 988 sentence. Not a crisis-directory expansion. 988 already in mental-health lessons.                                                                                                                                                                                                                                                                                                                                                            | **Clin 🟡 3**                           |
| Planner storage key `hmc-visit-planner-v2`. Read fallback + migrate from `hmc-visit-planner`. Rollback of P12 does not show raw ids.                                                                                                                                                                                                                                                                                                                                                                              | **PM 🟡 2**                             |
| Article TOC stays `scroll-mt-24` (header `min-h-[76px]`, not `h-20`).                                                                                                                                                                                                                                                                                                                                                                                                                                             | **UX 🟡 2 REJECT**                      |
| Phase 14 keeps sync server loaders + existing dual-pack fallback. Do not force `loadLessons.en.ts` as step 1.                                                                                                                                                                                                                                                                                                                                                                                                     | **Staff 🟡 4 REJECT**                   |
| Day 3 calendar unchanged (P8, 10, 11, 12). Do not move Header onto Day 2. If P7 lands early, **branch** P10; merge Day 3.                                                                                                                                                                                                                                                                                                                                                                                         | **PM 🟡 3 REJECT as stated**            |
| ClinicalCitationBlock does **not** add “ongoing annual schedule” (conflicts with 400-day fail).                                                                                                                                                                                                                                                                                                                                                                                                                   | **Clin 🟡 2 REJECT**                    |

**Rejected with justification (not in the table as a code change):**

- Header Tools dropdown / collapse to 4 items at `xl` — `getNavItems` is Home/Learn/Articles/Paths/Tools/Dashboard/Glossary/About. Spanish is `Rutas`, `Herramientas`, `Acerca de`, **not** “Planificador de visitas”. Compact-at-xl + `/es` overflow test is the fix. (`UX 🔴 1` as stated)
- “React 19 StrictMode double-mount breaks production PKCE” — StrictMode remount is **dev-only**. Consume-once is still mandatory because session-aware reset retriggers the effect while `?code=` remains. (`Staff 🔴 3` as stated)
- `scroll-mt-32` — 96px vs ~76px header is enough. (`UX 🟡 2`)
- “Clinically reviewed on an ongoing annual schedule” — 400-day validator already fails stale dates; canned cadence next to an old date is a new lie. (`Clin 🟡 2`)
- Force separate `loadLessons.en.ts` / `.es.ts` as the default P14 step — already the v3 fallback if webpack dual-packs; unconditional split blows the 3h timebox. (`Staff 🟡 4`)
- Move Phase 10/11 onto Day 2 — restacks visual chrome onto the quiz-unique day (CF-16). (`PM 🟡 3` as stated)
- Literal “100% pass rates” — 0-correct still fails; the 🔴 is UI-vs-DB pass disagreement + 1600% average. (wording only)

**Still in force from v3 (ROUND-1/2 / CF-1…CF-16), not re-opened:**
...
- Session-aware reset; confirm `type=recovery` forces reset-password; `useAuth().loading` only.
- Contact route keeps origin / rate-limit / honeypot; add body cap only.
...
- Phase 14 descoped; generators in scope; no Day-2 SSG rewrite; no client `loadLessons`.
...
- Planner per-type default map; `xl:flex` + compact-at-xl; HomeClient (not Hero) owns the video.
- Sentry `sendDefaultPii: false`; keep path slugs; no `dataCollection: {}`.
...
| Account deletion   | **Deploy `delete_user`**, then **local** `signOut({ scope: "local" })` in `finally` (ignore logout errors). Contact rows stay (not user-linked). Privacy copy must say that **and** how to request contact erasure (`privacy@healthmadeclear.com`).                                                                                                                                                                                        | GDPR/CCPA control must work on cookies, not only in Postgres.                              |
...
| ADV-09 `delete_user` missing                                             | 🔴      | 1 + **9 (client finally)**                                        | Throwaway RPC that deletes `auth.users` is a **gate**. Settings: local `signOut` in `finally`.                              |
...
| ADV-12 care-guide treatment voice                                        | 🔴      | 4                                                                 | Atypical-ACS chest-pain spec strings + one 988 sentence                                                                     |
...
| Visual header `2xl` → **`xl`** (not `lg`)                                | 🔴 UX   | 10                                                                | Compact-at-xl. 8 nav items. Playwright no overflow 1280/1440 **on `/en` and `/es`**. No Tools dropdown.                     |
...
| BUG-06 planner locale strings                                            | 🟡      | 12                                                                | Per-type default IDs matching `visitType`                                                                                   |
...
| ADV-16 Sentry `extra`/breadcrumbs                                        | 🟡      | 16                                                                | `sendDefaultPii: false`; clear user IP; strip query; **keep path slugs**; privacy `collectBodyErrors`                       |
...
| **Day 2** | 4, **6 (`015` + client, first)**, **5 (after unique live)**, 7 | Clinical honesty + quiz unique/upsert + then guest storage + achievements. **P6 before P5.**                                                       |
| **Day 3** | **8**, 10, 11, 12                                              | Citations/MDX, then visual/a11y chrome, search, planner. If 8 lands early, start 13.                                                               |
| **Day 4** | 13 (if leftover), **14 (descoped, 3h timebox)**, 15, 16        | Reading UX, **generator + client** split, small features, hardening. P14 is **not** the first task of the morning. Stop P14 at 13:00 if not green. |

**Slip protocol:** If Day 3 is not done by start of Day 4, **cut Phase 16 AppProviders/N+1** and any remaining P14 **path/glossary** generator work after lessons/quizzes client split is done. Never cut Phases 1–6. Never apply `015` without the Phase 6 client. Never merge Phase 5 while unique is missing.

**Parallelism:** Phase 2 can start while Phase 1 SQL is in review. Phase 4 is independent of 1–3. Phase 8 (Day 3) can start as a branch on Day 2 afternoon after P4 copy is stable; **merge on Day 3**. If P7 lands early, **branch** Phase 10 (do **not** merge Header onto Day 2). Phase 6 **must not apply `015` to production** until the upsert client is in the same Netlify deploy (or already live). Phase 5 **must not merge** until that unique exists. **P6 is larger this round** (units + `QuizClient` + dashboard ids) — still one PR, still before P5.
...
- `privacy.collectBodyErrors` — If crash reporting is enabled, we may receive the page path (including lesson or article slugs) and a technical error message. We do not send your name, email, or IP address in those reports. We strip URL query strings.
...
| `src/components/providers/AuthProvider.tsx`                           | After `supabase.auth.signOut()` succeeds, call `clearGuestProgress()`. Do **not** add a 2-hour migrate TTL (browse-then-signup is the product). Shared-kiosk confirm dialog is **out of scope**.                                                                                                                                                                                                                                                                                                                   |
...
- `AuthProvider.test.tsx` — `signOut` calls `clearGuestProgress` (mock).
...
- [ ] `signOut` clears `hmc_guest_*`. Login/signup shows `guestProgressWillSync`.
...
| `src/app/[locale]/dashboard/settings/SettingsClient.tsx` | After Phase 1, delete works. **Do not** call `AuthProvider.signOut` on this path (it is global logout + `router.push` and can throw). After `rpc("delete_user")` **success:** `try { await supabase.auth.signOut({ scope: "local" }) } catch { /* ignore user-not-found / session-missing */ } finally { clearGuestProgress(); router.push("/"); toast accountDeleted }`. Keep `deleting` true until finally. Rpc **error:** toast `deleteFailed`, `setDeleting(false)`, return (no sign-out). Unit test mocks `rpc("delete_user")`. Extra case: rpc ok + `signOut` rejects → still redirected, session mock empty. |
...
- `src/app/[locale]/dashboard/settings/SettingsClient.test.tsx` — rpc failure toast; success local signOut; **rpc ok + signOut reject → still `router.push("/")`**.
...
- [ ] Settings delete unit-tested, including rpc-ok + signOut-reject still redirects.
...
| File                                            | How                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/components/Header.tsx`                     | Replace nav `2xl:flex` with **`xl:flex`** (line ~124). Hamburger `2xl:hidden` → **`xl:hidden`** (~199, ~218, ~250). **Do not** use `lg:flex` for the 8-item inline nav — auth+utilities already `lg:flex`; adding labeled nav at 1024 overflows (visual audit allowed `lg` **or** `xl`; 1440 is `xl`). **Compact at `xl` (1280–1535):** logo tagline `hidden 2xl:block` (today `sm:block`); login `ButtonLink` icon-only at `xl` (text `2xl:inline` / `xl:hidden` on the label); **signup `ButtonLink` `2xl:hidden` → `xl:hidden`** (today “Crear cuenta” still shows at 1280); `NavLink` desktop padding `xl:px-2 xl:gap-1`. **Rule:** `< xl` (1280) = drawer; `xl+` = inline nav. Re-read all `2xl:` nav-visibility classes. Do not convert unrelated 2xl padding without checking. |
| `src/components/Header.test.tsx`                | Assert desktop nav class includes `xl:flex` and hamburger `xl:hidden`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**Playwright:** `e2e/visual.spec.ts` — viewport **1440×900** `/en` — `getByRole('navigation')` in header visible (not only drawer); hamburger hidden. Viewport **1280×800** and **1440×900**: evaluate header inner bar `scrollWidth <= clientWidth` (no horizontal overflow) on **both `/en` and `/es`**. Do **not** collapse nav to a Tools dropdown — `getNavItems` is the 8 short catalog labels (`Rutas`, `Herramientas`, `Acerca de`), not visit-planner names. `/en/this-page-does-not-exist` — home button visible, height ≥ 44 (bounding box).

### 13.3 Acceptance

- [ ] **1440px:** Learn/Articles/Tools/etc. inline; hamburger hidden.
- [ ] **1280px and 1440px:** header does not overflow (`scrollWidth <= clientWidth`) on `/en` **and** `/es`.
...
| `src/app/[locale]/tools/visit-planner/useVisitPlanner.ts`                  | Persist ids to **`hmc-visit-planner-v2`**. Hydrate: read v2 first; else read `hmc-visit-planner` (v1), migrate in memory, write v2. If value matches `/^(new-symptom\|medication\|followup):\d+$/` use as-is; else map old locale text → id via provided `questionCatalog`; else drop. Custom questions unchanged. Do not delete v1 until a later window (rollback of this PR still reads v1).                    |
| `src/app/[locale]/tools/visit-planner/VisitPlannerClient.tsx`              | Build catalog `{ id, text }[]` from `t.raw("plannerQuestions")` with index ids. Pass **`PLANNER_DEFAULTS_BY_TYPE[visitType]`** into `useVisitPlanner` (not `questions.slice(0, 2)` of translated strings). `changeVisitType(next)` sets selected ids to that map for `next`. Pass texts to step 2 by resolving ids. `useRef` on step `<h2 tabIndex={-1}>`; `useEffect` on `step` → `headingRef.current?.focus()`. |
...
| `src/app/[locale]/tools/visit-planner/useVisitPlanner.test.ts`             | ID persist; EN strings in storage migrate when catalog passed; **first load** defaults `new-symptom:2` + `new-symptom:3`; `changeVisitType("medication")` → medication defaults.                                                                                                                                                                                                                                  |
...
**Not** `new-symptom:0/1` (cause / which tests). **Not** a cross-type list like `medication:1` + `followup:3` while `visitType` is `new-symptom` — Step 2 would show unchecked boxes and Step 3 would show the wrong questions. `changeVisitType` applies **this map**, not `slice(0, 2)` of locale strings.
...
| `src/app/[locale]/articles/[slug]/ArticlePageClient.tsx`                | Body `max-w-prose` (~65ch) `leading-[1.75]`. Desktop `lg:` grid: sticky TOC `w-60` from `article.content.sections` titles, `position: sticky top-24`. TOC links `#section-slug`. Add `id` on each `<section>` or `h2`. **`scroll-mt-24`** on those targets (sticky header ~76px).                                                                                                                                                           |
...
- [ ] Article measure ~65ch; TOC on desktop; heading targets `scroll-mt-24`.
...
**Dependencies:** After correctness phases. **Day 4, not first thing in the morning.** Hard **3-hour** timebox: if analyzer/CI not green by **13:00**, stop; keep whatever lesson/quiz client split is done; drop remaining path/glossary generator work; proceed to Phase 15 then 16. Do **not** move this rewrite to Day 2.
...
- [ ] Lesson, article, care-guide have Print; `@media print` hides `no-print`. Printable surfaces use `print-color-adjust: exact`.
...
- `errorReporting.test.ts` — `reportServerError`: 6th call inside 10s does not `fetch`.
...
- [ ] Sentry client: `sendDefaultPii: false`; no user IP/email; query stripped; **path slugs kept**.
- [ ] Privacy page includes `collectBodyErrors`.
- [ ] Server reporter attempts ingest when DSN set; 6th error in 10s does not `fetch`.
...
**Edge cases:** see Phase 12 hydrate. Defaults from `PLANNER_DEFAULTS_BY_TYPE` matching current `visitType` (not a cross-type list). `changeVisitType` applies that map.
...
- [ ] `delete_user` exists; Settings deletion removes auth user **and** clears local session even if GoTrue logout errors (spot-check on a throwaway account).
...
- [ ] 1440px header shows desktop nav (`xl`, not `lg`). Header does not overflow at 1280 or 1440 on `/en` **and** `/es`.
...
P6 (015 + upsert client, one window) ──► unique live ──► P5 (guest keys; after unique)
P4 independent; P8 Day 3 (branch may start after P4)
P7 **no** loadLessons; generated lessonMeta only
P10–P13 independent (P13 video = HomeClient order **+** preload/reduced-motion)
P14 generators + client path split (Day 4, 3h timebox, not morning-first)
P15 after P8/P13
P16 last (**no** HomeClient)
```
