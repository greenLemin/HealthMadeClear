# HealthMadeClear Launch Revamp — Implementation Plan v7

**Status:** Contract for all implementation work in the 4-day launch window. **Supersedes `REVAMP/PLAN.v6.md` (v6).** v5, v4, v3, v2, and v1 are historical.  
**Date:** 2026-08-27  
**Inputs:** `REVAMP/AUDIT-CODE.md` (file audit + independent adversarial/live-DB audit), `REVAMP/AUDIT-VISUAL.md`, `REVAMP/CRITIQUES/ROUND-1.md`, `REVAMP/CRITIQUES/ROUND-2.md`, `REVAMP/CRITIQUES/ROUND-3.md`, `REVAMP/CRITIQUES/ROUND-4.md`, `REVAMP/CRITIQUES/ROUND-5.md`, `REVAMP/CRITIQUES/ROUND-6.md` (panel + Cursor/Grok second opinion)  
...
v6 already incorporated ROUND-1/2/3/4/5 + CF-1…CF-37. Do not re-litigate those without a new finding. This file starts from that contract.

---

## Changelog from v6

Every row is a delta vs `REVAMP/PLAN.v6.md`. Critiques: `Staff` / `Sec` / `UX` / `Clin` / `PM` = panel ROUND-6; `CF-N` = combined flaws in the Cursor/Grok second opinion on ROUND-6 (CF-38…CF-42).
...
Every row below is a delta vs `REVAMP/PLAN.v5.md` that v6 already shipped. Critiques: `Staff` / `Sec` / `UX` / `Clin` / `PM` = panel ROUND-5; `CF-N` = CF-33…CF-37. Do not re-do these as v7 work.
...
| Account deletion   | **Write `delete_user` in `014`, apply `014` only after Gate 0 + Gate 1 (P9 live).** Then **local** `signOut({ scope: "local" })` in `finally` (ignore logout errors). **Then `resetLocalProgress()`** (empty AppProviders React state **then** prefix-scan `clearLocalHealthData()`). Same wipe on **logout** via `AuthProvider.signOut` **`try/finally`** (not success-only). Middleware expires `sb-*auth*` cookies when `getUser()` **resolves with `error`**, on the **returned** response (copy onto dashboard 307). Contact rows stay (not user-linked). Privacy copy must say that **and** how to request contact erasure (`privacy@healthmadeclear.com`). | GDPR/CCPA control must work on cookies **and** shared-device storage, not only in Postgres. HEAD Settings already calls the RPC — applying 014 before P9 turns a failing button into a kiosk leak. `removeItem` without resetting mounted `AppProviders` is rewritten by the persist effect. A hardcoded key list drifts. Success-only logout skips the wipe when GoTrue errors. Expire-on-throw logs everyone out during an outage. Expire-on-`supabaseResponse` then `redirect()` drops `Set-Cookie`. |
...
| ADV-09 `delete_user` missing                                             | 🔴      | 1 + **9 (client finally + prefix health wipe + AuthProvider `signOut` finally + middleware cookie expire on returned response)** + **Gate 1: P9 live before 014 apply** | Throwaway RPC that deletes `auth.users` is a **gate**. Settings: local `signOut` in `finally` + `resetLocalProgress`. `AuthProvider.signOut` same `try/finally`. Middleware expires `sb-*` cookies on resolved `getUser` error **on the response that is returned** (copy onto dashboard 307). **Do not `db push` 014 until P9 is live.** |
...
| TEST-01 auth form unit tests                                             | 🟡      | 9                                                                                                                                                                       | **Day 1 after Phase 2, before 014 apply** (required — delete wipe + AuthProvider `finally` + middleware cookie expire on dashboard 307 live here).                                                                                                                                                                                        |
...
| Print CTAs lesson/article/care-guide                                     | 🟡 feat | 15                                                                                                                                                                      | Print-only 911/988 line on care-guide. Red banner stays `no-print`. Print-only educational disclaimer footer **with print-time date**. Poison Help is in `whenInDoubtBody` (Phase 4), not a third print hotline unless it already fits.                                                                                                   |
...
| Day       | Phases                                                                                                               | Theme                                                                                                                                                                                                                                                                     |
| --------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Day 1** | Write `014` + **Gate 0**; 2; 3 (env-gate); **9 (after P2, live on Netlify)**; **then apply `014` (Gate 0 + Gate 1)** | Security + auth + legal copy. **P9 must be live before `delete_user` is callable.** Contact PostgREST INSERT stays open until that apply (same day). **Do not announce launch until 1–3 are on production.** **Do not apply `015`.** P9 is **not** optional.              |
...
**Parallelism:** Phase 2 can start while Phase 1 SQL is in **review** (writing `014` does not apply it). **Phase 3 env-gate file may merge (and should) before `014` is applied** — Gate 0 still required at apply time. **Gate 1:** do **not** `db push` `014` until Phase 9 is merged **and** that commit’s Netlify production deploy is Ready. HEAD `SettingsClient` already calls `rpc("delete_user")`; applying 014 first turns a failing button into a kiosk leak. **Not concurrent.** **Phase 4 is independent of 1–3 and of 6** — it may run anytime on Day 2 (including in parallel with P6). Do **not** treat listed calendar order as a single-threaded queue that puts copy work ahead of 015. Phase 8 (Day 3) can start as a branch on Day 2 afternoon after P4 copy is stable; **merge on Day 3**. Day 3 PRs (8/10/11/12) may overlap locally while CI runs — that is already one-phase-one-PR, not a mega-merge. If P7 lands early, **branch** Phase 10 (do **not** merge Header onto Day 2). Phase 6 **must not apply `015` to production** until the upsert client is in the same Netlify deploy (or already live). Phase 5 **must not merge** until that unique exists. **P6 is larger this round** (units + `QuizClient` + dashboard ids) — still one PR, still before P5. **P9 after P2 and before 014 apply** so delete **and logout** wipe **and** stale-JWT cookie expire exist before `delete_user` is live and before P5 makes localStorage canonical.
...
**Gate 1 (blocking — before any `014` `db push`):** Phase 9 is **merged to `main` and that commit’s Netlify production deploy is Ready**. Proof in the apply note: Netlify deploy SHA matches the P9 merge (Settings `try/finally` + prefix wipe + middleware copies cookies onto dashboard 307). HEAD `SettingsClient.tsx` already calls `rpc("delete_user")` then `AuthProvider.signOut()`; applying 014 first makes account deletion succeed on the **old** client (no local wipe, leftover JWT). **Do not** apply 014 “concurrently” with P9 — that is a race. **Do not** split `delete_user` into a later migration (014 still supersets 009). ADV-10 public INSERT remains open until this apply; that is accepted same-day cost of Gate 1. Writing/reviewing 014 and repairing `009`–`013` **as applied** may happen before Gate 1; only `db push` waits.
...
- [ ] **Gate 1:** Phase 9 is live on Netlify production (deploy SHA = P9 merge) **before** `014` `db push`. HEAD Settings `rpc("delete_user")` is not callable against production until that wipe exists.
...
**Dependencies:** Env-gate file **must** merge before `014` apply (Gate 0 still at push). Privacy copy may ship before or after 014. **014 apply waits on Gate 1 (P9 live)** — do not treat “P3 after P1 apply” as a reason to push 014 first. Must not go to Netlify without the key if 014 is already live.
...
| `whenInDoubtTitle`, `whenInDoubtBody`                                              | No “go to urgent care” as a directive. Same 988 sentence as `emergencyBody`. **Also US Poison Help `1-800-222-1222`** (resource, not “skip 911”).                                                                                            |
...
- `homeCarePediatricNote` ES: "Cualquier temperatura de 100.4°F (38°C) o más en bebés menores de 3 meses, o signos de deshidratación en niños pequeños (sin pañales mojados por 8 horas, llanto sin lágrimas, ojos hundidos, o si es muy difícil despertarlos), se tratan como emergencias médicas en los Estados Unidos — no como observación en el hogar. En los Estados Unidos, llame al 911."
...
- Do **not** use `somnolencia extrema` (over-formal). The “muy difícil despertarlos” phrasing is the paste.
...
- `whenInDoubtBody` EN: "If you think you may be having an emergency, use local emergency services. In the United States that is 911. For a mental health crisis or thoughts of self-harm in the United States, call or text 988 (Suicide & Crisis Lifeline). For questions about accidental double doses, medicine mix-ups, or suspected poisoning in the United States, call Poison Help at 1-800-222-1222 (free, confidential, 24/7). This site cannot triage you."
- `whenInDoubtBody` ES: "Si cree que puede estar teniendo una emergencia, use los servicios de emergencia locales. En los Estados Unidos eso es el 911. Para una crisis de salud mental o pensamientos de autolesión en los Estados Unidos, llame o envíe un mensaje de texto al 988 (Línea de Prevención del Suicidio y Crisis). Para preguntas sobre dosis dobles accidentales, confusión de medicamentos o sospecha de intoxicación en los Estados Unidos, llame a Poison Help (Centro de Control de Intoxicaciones) al 1-800-222-1222 (gratuito, confidencial, 24/7). Este sitio no puede hacer un triaje."
- `emergencyBody` EN: keep physical life-threatening examples, plus the same 988 sentence. Not a directive to “go to the ER now” for every worry. Poison Help belongs in `whenInDoubtBody`, not a second 911 replacement.
...
- [ ] `whenInDoubtBody` names US Poison Help `1-800-222-1222` in EN+ES (keep 911 and 988).
- [ ] `homeCarePediatricNote` (EN+ES) names infant fever (<3 months) at **100.4°F (38°C) or higher** and pediatric dehydration; rendered under the home-care card. Checklist has no bare “Low fever” / “Fiebre baja.” UI string does **not** say “rectal.” ES does **not** use `somnolencia extrema`.
...
4. **Poison Help on real files:** In `understanding-prescription-labels.mdx` EN+ES, add a warning/takeaway:
   ...
   - ES: `"Para preguntas sobre dosis dobles accidentales, confusión de medicamentos o sospecha de intoxicación, llame a Poison Help (Centro de Control de Intoxicaciones de EE. UU.) al 1-800-222-1222 (gratuito, confidencial, 24/7)."`
     Grep `pain-medications-safely` for `222-1222`; add only if missing. Do not invent new lesson files.
...
- `content/lessons/en/understanding-prescription-labels.mdx` + ES: file contains `222-1222`. Same assertion on `pain-medications-safely` EN+ES.
...
- [ ] `understanding-prescription-labels` EN+ES names `1-800-222-1222`. `pain-medications-safely` EN+ES still does (verify). No invented lesson slugs.
- [ ] EN/ES.
...
**Goal:** Signup does not enumerate emails; expired sessions get a clear message; critical auth forms have unit tests; **account delete and (via the same util) logout wipe shared-device health storage without the persist-effect rewrite;** leftover `sb-*` cookies expire when `getUser()` resolves with an auth error **on the response that is returned** (dashboard 307 copies cookies). **This PR is Gate 1 for `014` apply.**
...
| `src/lib/supabase/middleware.ts`                         | After `getUser()`: if the call **resolves** with `error`, expire cookies whose names match `/^sb-.*-auth-token/` (include chunked `sb-*-auth-token.0` suffixes) on the **response that is returned** (`Max-Age=0; Path=/`). If `getUser()` **throws** (outage), **keep** cookies; existing dashboard-unauthenticated behavior. Guest: `user===null`, no `error`, no auth cookies → no-op (do **not** redirect `/learn` to `login?error=session_expired`). Dashboard `!user`: `const redirectResponse = NextResponse.redirect(loginUrl)`; **copy** `supabaseResponse.cookies.getAll()` onto `redirectResponse` (`cookies.set` each); **then** if resolved `error`, expire `sb-*` auth cookies on **`redirectResponse`**; `return redirectResponse`. **Never** `return NextResponse.redirect(loginUrl)` without copying. `LoginForm` has no auto-redirect — do not add one. Paste helper: `expireSbAuthCookies(res)` loops `request.cookies.getAll()` names matching the regex and `res.cookies.set(name, "", { maxAge: 0, path: "/" })`. |
...
- [ ] Middleware expires `sb-*auth*` cookies when `getUser()` **resolves with `error`**, including on the **dashboard 307** (cookies copied onto `NextResponse.redirect`). Thrown `getUser` does **not** expire cookies. Guests without auth cookies are not sent to `session_expired`.
- [ ] `clearLocalHealthData` prefix-scans both stores; planted `hmc-future-tool` is gone; theme kept.
...
**Playwright:** 1440 `/en` — primary CTA `Start learning` bounding box `y + height < 900`. Article `/en/articles/understanding-your-eob` — TOC visible at lg; paragraph max width ≤ 720px (evaluate); at `lg` the article `<main>` appears before the TOC `<aside>` in DOM order (`compareDocumentPosition`); **distance from `<main>` content box `right` to `<aside>` content box `left` ≤ 80px** (no 1440 dead zone). Quiz: optional screenshot stability skip.
...
- [ ] Article measure ~65ch; TOC on desktop; heading targets `scroll-mt-24`. Article `<main>` precedes TOC in DOM (keyboard/SR reach the article first). At 1440, TOC sits within 80px of the prose column (`max-w-5xl` shell, not `max-w-container`).
...
- [ ] Lesson, article, care-guide have Print; `@media print` hides `no-print`. Printable surfaces use `print-color-adjust: exact`. Care-guide print includes a print-only 911/988 line **and** a print-only educational disclaimer footer **with print-time date**; red banner stays `no-print`.
...
**UX flow:** `lg+` two columns **inside `max-w-5xl`**, not inside `max-w-container` (1440px). TOC sticky; click scrolls to `id`. Mobile: no TOC (sections still have headings). Gap between prose and TOC is `lg:gap-12` / `xl:gap-16` (~48–64px), not a stretched `1fr`.
...
**A11y:** TOC `aside`/`nav` `aria-label={t("onThisPage")}`; skip link already to `#main-content`. **DOM order:** article `<main>` first, TOC second; CSS grid for visual right column. **No scroll-spy required this window** (IntersectionObserver is polish, not WCAG). **Layout:** parent `mx-auto max-w-5xl lg:grid lg:grid-cols-[minmax(0,1fr)_240px]`. Do not use `lg:grid-cols-[1fr_240px]` on the 1440px site shell.
...
- [ ] `delete_user` exists; Settings deletion removes auth user **and** clears local session even if GoTrue logout errors (spot-check on a throwaway account). **Prefix `hmc[-_]` storage gone in both stores; theme/locale kept.** Persist effect does not restore the previous user. Logout (`AuthProvider.signOut` `finally`) uses the same wipe. Middleware expires `sb-*auth*` cookies when `getUser()` **resolves with `error`** (not on throw), **including on the dashboard 307** (cookies copied from `supabaseResponse`).
- [ ] Gate 0: Netlify `SUPABASE_SERVICE_ROLE_KEY` proven before 014 `db push`. **Gate 1: Phase 9 live on Netlify production before 014 `db push`.** Contact `/api/contact` stream-caps bodies (~10KB) even without `Content-Length`. Empty body and malformed JSON return **400**; oversize **413**; those paths do not `reportServerError`.
...
- [ ] Care guide `whenInDoubtBody` names US Poison Help `1-800-222-1222` in EN+ES. `understanding-prescription-labels` EN+ES names the same number.
...
- [ ] Article body ~65ch + desktop TOC (`<main>` before TOC in DOM; **`max-w-5xl` shell**; 1440 TOC–prose gap ≤ 80px).
...
```
P1 Gate 0 (Netlify service role proven) ──► P3 env-gate file (may merge first)
P2 (auth routes, session-aware reset) ──► P9 (delete wipe + AuthProvider signOut finally + middleware cookie copy on dashboard 307) ──► Gate 1 (P9 live on Netlify)
...
P4 independent (parallel with P6 on Day 2); P8 Day 3 (branch may start after P4; Poison Help MDX here)
P7 **no** loadLessons; generated lessonMeta only; **no** client getMessages; quiz sideEffects take locale
P10–P13 independent (P13 video = HomeClient order **+** preload/reduced-motion; P13 TOC = max-w-5xl shell)
P14 generators + client path split (Day 4, 3h timebox, not morning-first; **not** message catalogs)
P16 19.1 (CSP + Sentry PII, strip ? and #, beforeBreadcrumb drop ui.input, ingest AbortSignal.timeout(2000)) after P14 timebox
...
ADV-10, ADV-09, ADV-15 → P1 write `014`; **apply after Gate 1 (P9 live)**  
...
ADV-12, A11Y-01, MED-03, Poison Help care-guide → P4  
ADV-02, ADV-14 → P7  
MED-01, MED-02, Poison Help prescription MDX → P8  
ADV-13, ADV-05, TEST-01, shared-device wipe, middleware cookie copy → P9 (Day 1, **Gate 1 before 014 apply**)  
Visual 1, 8, 10 → P10  
