# HealthMadeClear Launch Revamp — Implementation Plan v3

**Status:** Contract for all implementation work in the 4-day launch window. **Supersedes `REVAMP/PLAN.v2.md` (v2).** v1 (`REVAMP/PLAN.md`) is historical.  
**Date:** 2026-08-27  
**Inputs:** `REVAMP/AUDIT-CODE.md` (file audit + independent adversarial/live-DB audit), `REVAMP/AUDIT-VISUAL.md`, `REVAMP/CRITIQUES/ROUND-1.md`, `REVAMP/CRITIQUES/ROUND-2.md` (panel + Cursor/Grok second opinion)  
...
v2 already incorporated ROUND-1 + combined flaws CF-1…CF-7. Do not re-litigate those without a new finding. This file starts from that contract.

---

## Changelog from v2

Every row is a delta vs `REVAMP/PLAN.v2.md`. Critiques: `Staff` / `Sec` / `UX` / `Clin` / `PM` = panel ROUND-2; `CF-N` = combined flaws in the Cursor/Grok second opinion on ROUND-2 (CF-8…CF-16). ROUND-1 IDs named when a v2 leftover is closed.

| Change                                                                                                                                                                                                                                                                                                                                  | Driven by                                                        |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| This file is the contract; v2 is historical.                                                                                                                                                                                                                                                                                            | Process                                                          |
| **Phase 6 before Phase 5** on Day 2 (two PRs, ordered). Unique `(user_id, quiz_id)` + `mutations.ts` upsert must be live before guest-storage PR. Delete v2 claim that `ON CONFLICT` is a no-op without a unique.                                                                                                                       | **Staff 🔴 1 PARTIAL**, **PM 🔴 1 PARTIAL**, **CF-8**, **CF-16** |
| Do **not** mega-merge Phase 5 + Phase 6 into one PR (guest storage + SQL dedupe + dashboard aggregation is unreviewable).                                                                                                                                                                                                               | **PM 🔴 1 REJECT as stated**, **CF-16**                          |
| Phase 8 (citations/MDX) moves to **Day 3 morning**. Day 2 = P4, **P6, P5**, P7. Scope **not** cut.                                                                                                                                                                                                                                      | **PM 🔴 2 PARTIAL**                                              |
| Planner defaults are **per visit type**, matching `visitType`. Initial type stays `"new-symptom"`. Map: `new-symptom` → `["new-symptom:2","new-symptom:3"]`; `medication` → `["medication:1","medication:3"]`; `followup` → `["followup:0","followup:3"]`. `changeVisitType` applies that map (not `slice(0,2)` of translated strings). | **Staff 🔴 2**, **CF-9**                                         |
| `ResetPasswordClient`: session gate is `useAuth().loading` / `useAuth().user` **only**. Never `useAuthFormState().loading` (that is submit-in-flight, starts `false`).                                                                                                                                                                  | **Staff 🟡 4**, **CF-10**                                        |
| Sentry: explicit `sendDefaultPii: false`; clear `event.user.ip_address` (and email/username). Strip query (already v2). **Do not** rewrite `/learn/[slug]` → `/learn/*`. **Do not** pass a `dataCollection: {}` object (v10 permissive defaults include IP). Privacy: new `collectBodyErrors`.                                          | **Sec 🔴 1 PARTIAL**                                             |
| Header nav: `xl:flex` / hamburger `xl:hidden` (**not** `lg:flex`). Compact at `xl`: tagline `hidden 2xl:block`; login icon-only until `2xl`; `NavLink` `xl:px-2`. Nav has **8** items, not 6. Playwright: 1440 nav visible; 1280 and 1440 header `scrollWidth <= clientWidth`.                                                          | **UX 🔴 1**, **CF-12**                                           |
| Phase 13 video lives in `src/app/[locale]/HomeClient.tsx` (**not** `Hero.tsx`). `<sm` DOM: `<Hero />` (CTAs) **then** `<video>`. `Hero.tsx` = type clamp + TrustBanner slot only.                                                                                                                                                       | **UX 🔴 2**, **CF-11**                                           |
| `scenarioSoreThroatBody` spec **is** the airway/abscess red-flag paragraph (EN+ES). v2 rule without example copy was not enough.                                                                                                                                                                                                        | **Clin 🔴 1**, **CF-14**                                         |
| Quiz persist: skip `quiz_attempts` write when `score <= existing.score`; **always** `handleQuizAttemptSideEffects` (daily_log + streak).                                                                                                                                                                                                | **Staff 🟡 3**, **CF-13**                                        |
| `handle_new_user`: after trim, `regexp_replace(..., '[[:cntrl:]]', '', 'g')`, then substring 100. POSIX class; not `[\\x00-\\x1F]` (unreliable in Postgres POSIX).                                                                                                                                                                      | **Sec 🟡 2**                                                     |
| `AuthProvider.signOut` calls `clearGuestProgress()`. Login/signup copy: browser progress attaches to the account. **No** 2-hour migrate TTL.                                                                                                                                                                                            | **Sec 🟡 3 PARTIAL**                                             |
| `scripts/bundle-lessons.ts` emits `src/data/lessonMeta.ts` (`id` + `level` only). P7 consumes it; P14 must keep the emit. Test vs EN `level === "beginner"`. No hand-written ID list.                                                                                                                                                   | **Staff 🟡 5**, **CF-15**                                        |
| Article TOC targets: `scroll-mt-24` on section/h2 ids.                                                                                                                                                                                                                                                                                  | **UX 🟡 3**                                                      |
| Search `aria-live`: mount empty node on dialog open; debounce 350ms; **mutate text**, do not remount the live region.                                                                                                                                                                                                                   | **UX 🟡 4**                                                      |
| `homeCareBody`: clinician/pharmacist before OTC, especially children, pregnancy, chronic conditions. Still not “take this drug.”                                                                                                                                                                                                        | **Clin 🟡 2**                                                    |
| Phase 14 hard **3-hour** timebox on Day 4. If single-locale client split not green in CI by 13:00, drop remaining path/glossary generator work (existing slip) and go to P15/P16.                                                                                                                                                       | **PM 🟡 3**                                                      |
| CSP: `pref-bootstrap.js` is **external**. Keep `'unsafe-inline'` for Next App Router inline bootstrap. Do not hash the external file as a substitute.                                                                                                                                                                                   | **Sec 🟢 4 REJECT as stated**                                    |

**Rejected with justification (not in the table as a code change):**

- Mega-merge Phase 5 + Phase 6 into one PR — review dump; order the two PRs instead. (`PM 🔴 1` / `Staff 🔴 1` as stated)
- Sentry slug-masking `/learn/*` as launch 🔴 — same path class already disclosed for GA; kills routing debug. SDK v10 does **not** send IP by default. (`Sec 🔴 1` as stated)
- 2-hour guest-migrate TTL — deletes browse-then-signup. (`Sec 🟡 3` as stated)
- Header `lg:flex` for inline nav — 8 labeled items + auth already at `lg` overflow at 1024. (`UX 🔴 1` `lg` option)
- SHA-256 hashes of `pref-bootstrap.js` to drop `'unsafe-inline'` — that file is not inline; Next inline scripts are. (`Sec 🟢 4`)
- Compact TrustBanner on mobile — already specified in v2 Phase 8. (`Clin 🟡 3` stale)
- Equal-score retake (100% then 100%) skips side effects — v2 used `<`, not `<=`. (`Staff 🟡 3` example)
- Apply `015` on Day 1 — Round 1 deadlock (authenticated `.insert()` → `23505`). Unchanged.

**Still in force from v2 (ROUND-1 / CF-1…CF-7), not re-opened:**
...
- Session-aware reset; confirm `type=recovery` forces reset-password.
...
| Visual header `2xl` → **`xl`** (not `lg`)                            | 🔴 UX   | 10                                                                | Compact-at-xl. 8 nav items. Playwright no overflow 1280/1440.                                         |
...
| Hero type scale + home video order                                   | 🟡      | 13                                                                | `Hero.tsx` clamp. **`HomeClient.tsx`:** `<sm` Hero then video.                                        |
...
- `whenInDoubtBody` EN: "If you think you may be having an emergency, use local emergency services. In the United States that is 911. This site cannot triage you."

Keep the four cards (home / primary / urgent / emergency) as **definitions**, not a decision tree that outputs an action. Each scenario: (1) how clinicians evaluate that presentation in different settings, (2) warning signs that are emergencies, (3) the not-advice line. **The sore-throat example above is the warning-sign requirement — do not ship a clinic-only sentence.**
...
- `CareGuideClient.test.tsx` — render with `next-intl` provider pattern from `MedicalDisclaimer.test.tsx`. Assert sore-throat body matches `/drool|swallow|breath/i` (EN) and the ES file contains the matching emergency terms (not only `clínica`).
...
**Rationale:** Unique `(user_id, quiz_id)` makes `.insert()` on retake fail (`23505`). Optimistic score without rollback is a lie. Guest migrate already upserts — authenticated path does not. **This unique also fixes today's guest-migrate `42P10`.**

**Complexity:** Medium  
**Risk:** Medium (data + deploy coupling)  
**Merge order: BEFORE Phase 5.**  
**Dependencies:** Phase 1 **`014` applied**. **`015` is applied in this phase, in the same production window as the upsert client.** Do not apply `015` while `mutations.ts` still `.insert()`. **Ship this phase before Phase 5.** Guest quiz upsert in HEAD already needs this unique (42P10 today).
...
| `src/hooks/useProgress/mutations.ts`                | `saveQuizAttempt`: keep previous attempts snapshot; on error restore and toast; use `.upsert(..., { onConflict: QUIZ_ATTEMPTS_ON_CONFLICT })` with `ignoreDuplicates: false`. If `existing && score <= existing.score`, **skip the `quiz_attempts` write** but **still** `await handleQuizAttemptSideEffects(...)` (daily_log + streak). If new score wins, send new `answers`. |
| `src/lib/supabase/schema.ts`                        | Use `QUIZ_ATTEMPTS_ON_CONFLICT` from Phase 1.                                                                                                                                                                                                                                                                                                                                   |
| `src/hooks/useProgress/mutations.test.ts`           | Error path restores previous; upsert called not insert; `score <= existing` skips write and **still** calls side effects.                                                                                                                                                                                                                                                       |
...
- `mutations.test.ts` — already documents no revert; **change expectation** to revert on error. **New:** lower (and equal) score does not call `.upsert`/`.insert`; **does** call side-effects helper (mock).
...
**Playwright:** `e2e/visual.spec.ts` — viewport **1440×900** `/en` — `getByRole('navigation')` in header visible (not only drawer); hamburger hidden. Viewport **1280×800** and **1440×900**: evaluate header inner bar `scrollWidth <= clientWidth` (no horizontal overflow). `/en/this-page-does-not-exist` — home button visible, height ≥ 44 (bounding box).

### 13.3 Acceptance

- [ ] **1440px:** Learn/Articles/Tools/etc. inline; hamburger hidden.
- [ ] **1280px and 1440px:** header does not overflow (`scrollWidth <= clientWidth`).
...
| `src/app/[locale]/HomeClient.tsx`                                       | **Owns `/HMC_Video.mp4`.** Today the video block is **above** `<Hero />`. **`<sm`:** render `<Hero />` first, then the video container (CSS `order` or DOM swap). `sm+` may keep video above or beside — do not bury desktop layout. Do not move the file into `Hero.tsx`. Phase 16 still sets `preload` / reduced-motion on this same element. |
...
- [ ] 390px: primary CTAs above video (`HomeClient` order); TrustBanner one line.
...
- [ ] 1440px header shows desktop nav (`xl`, not `lg`). Header does not overflow at 1280 or 1440.
- [ ] Footer/legal/glossary letters/checklist/pills meet 44px on 390px.
- [ ] Search announces result counts; groups by type.
- [ ] Planner survives EN↔ES; step focus moves; first-load selected IDs match `new-symptom` defaults.
- [ ] 390px home: primary CTAs above autoplay video (`HomeClient` order).
...
P10–P13 independent (P13 video = HomeClient)
...
ADV-01 apply + client, ADV-04 → P6 **then** BUG-03, BUG-04, ADV-03 → P5  
...
Visual 3–4, 7, quiz CLS, cards, paths → P13 (`HomeClient` video order, not `Hero.tsx`)  
PERF-01 → P14  
