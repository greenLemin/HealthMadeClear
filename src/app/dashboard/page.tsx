...
export default function Dashboard() {
  const { completedLessons, locale, recentLessons } = useAppState();
  ...
  const startedPathCount = getStartedPathCount(completedLessons, lessons, learningPaths);
  ...
  const activePath = learningPaths
          ...
          <div className="card">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-headline-lg text-primary">{completedCount} / {totalLessons}</div>
              </div>
            </div>
            <div className="progress-bar mb-3 h-3" role="progressbar" aria-valuenow={progressPercentage} aria-valuemin={0} aria-valuemax={100} aria-label={locale === "es" ? "Progreso general" : "Overall progress"}>
              <div className="progress-fill" style={{ width: `${progressPercentage}%` }} />
            </div>
            <div className="text-sm text-on-surface-variant">{progressPercentage}% {copy.dashboard.ofLibrary}</div>
          </div>
            ...
            <div className="grid gap-0 md:grid-cols-[0.8fr_1.2fr]">
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <Link href={nextLesson ? `/learn/${nextLesson.id}` : "/learning-paths"} className="btn-primary inline-flex items-center justify-center">
              ...
              {learningPaths.slice(0, 3).map((path) => {
                ...
                return (
                  <div key={path.id}>
                    ...
                    <div className="progress-bar mb-2" role="progressbar" aria-valuenow={progress.percentage} aria-valuemin={0} aria-valuemax={100} aria-label={`${path.title} progress`}>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="card">
            ...
            <div className="space-y-4">
              {completedCount === 0 ? (
                <div className="text-body-md text-on-surface-variant">
                  {copy.dashboard.noRecentMilestones}
                </div>
              ) : (
                completedLessons.slice(0, 3).map((lessonId) => {
                  ...
                  return (
                    <div key={lesson.id} className="flex items-start gap-3 rounded-lg bg-surface-container-low p-4">
                      ...
                      <div>
                        <div className="text-label-md text-on-surface">{copy.common.completed}: {lesson.title}</div>
                        <div className="text-sm text-on-surface-variant">{lesson.category}</div>
                      </div>
                    </div>
                  );
                })
