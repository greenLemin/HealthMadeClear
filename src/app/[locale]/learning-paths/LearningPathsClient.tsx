...
type PathHeaderProps = {
  ...
  t: ReturnType<typeof useTranslations<"paths">>;
  markPathStarted: (id: string) => void;
};

function PathHeader({ path, locale, isStarted, nextLesson, t, markPathStarted }: PathHeaderProps) {
  return (
...
function PathContent({ path, glossaryTerms }: PathContentProps) {
  if (!path.content?.sections) return null;
...
type PathProgressCardProps = {
  progress: { percentage: number; completedCount: number; totalCount: number };
  nextLesson?: LessonListItem;
  t: ReturnType<typeof useTranslations<"paths">>;
  tCommon: ReturnType<typeof useTranslations<"common">>;
};

function PathProgressCard({ progress, nextLesson, t, tCommon }: PathProgressCardProps) {
  return (
...
type PathLessonListProps = {
  pathLessons: LessonListItem[];
...
function PathLessonList({ pathLessons, completedLessons, pathId, t, markPathStarted }: PathLessonListProps) {
  return (
      ...
      <div className="space-y-3">
        {pathLessons.map((lesson) => (
          <Link
            ...
            onClick={() => markPathStarted(pathId)}
          >
            <div>
    ...
    <Reveal delay={Math.min(index * 0.05, 0.2)}>
      <section id={path.id} className="surface-card-strong scroll-mt-24 px-6 py-6 md:px-8 md:py-8">
        <PathHeader
          ...
          t={t}
          markPathStarted={markPathStarted}
        />

        <div className="mt-8 grid gap-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
          <div className="space-y-5">
            <PathContent path={path} glossaryTerms={glossaryTerms} />
            <PathProgressCard progress={progress} nextLesson={nextLesson} t={t} tCommon={tCommon} />
          </div>
