...
export default function LearningPathCard({ path, lessons, progress }: LearningPathCardProps) {
  ...
  const pathProgress =
    progress ||
    (() => {
      const completed = pathLessons.filter((l) => completedLessonsSet.has(l.id)).length;
  ...
  const nextLesson = pathLessons.find((l) => !completedLessonsSet.has(l.id)) ?? pathLessons[0];
